...
const getGroups = (req, res) => {
  ...
  if (groups) {
    ...
    res.status(404).json({ error: 'No group in worksapce' });
  }
  /**
   * @openapi
   * components:
...
const createGroup = (req, res) => {
  const { group_icon, group_title, browserTab_favIconURL, browserTab_title, browserTab_url, item_id, sourceGroup_id } = req.body;
  ...
  if (group_icon && group_title) {
    ...
    res.status(201).json({ message:"Group created with tab from other group successfully",group_id });
    /**
    * @openapi
    * components:
    *   responses:
    ...
    */
  } else if (browserTab_favIconURL && browserTab_title && browserTab_url&&group_title &&group_icon) {
    ...
    newGroup = {
      ...
      group_icon,
      group_title,
      items: { browserTab_favIconURL, browserTab_title, browserTab_url, item_id }
    };
    res.status(201).json({  message: "Sidebar group created successfully",group_id, item_id });
    /**
     * @openapi
     * components:
     *   responses:
   ...
   //GroupCreatewithGroupTabtoBlank
    ...
    res.status(201).json({  message: "Group created at blank space successfully",group_id });
    /**
     * @openapi
     * components:
     *   responses:
